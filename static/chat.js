...
function connectWebSocket(realId) {
    if (!listingId || !realId) return;
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const socketUrl = `${protocol}//${window.location.host}/ws/chat/${listingId}/${realId}/`;
...
document.addEventListener('DOMContentLoaded', async () => {
    const token = localStorage.getItem('accessToken');
    if (!localStorage.getItem('userId') && !token) return window.location.href = 'login.html';
