...
const isAuthenticated = Boolean(currentUserId || accessToken);
...
scrollTopBtn.addEventListener('click', () => {
  ...
  (async function checkAdminLink(){
    ...
    if(!isAuthenticated) return; // не залогінений
  ...
  loadCategories().then(loadLocations).then(loadFavoriteIds).then(loadListings);
  ...
  async function loadListings() {
    try {
      ...
      listings = normalizeApiList(await res.json());
  ...
  async function loadFavoriteIds() {
    ...
    try {
      ...
      if (!res.ok) return;
      const data = normalizeApiList(await res.json());
      favoritesIds = data.map(item => Number(item.id));
      if (favoritesKey) localStorage.removeItem(favoritesKey);
    } catch (err) {
      console.warn('Помилка синхронізації обраного', err);
    }
  }
  ...
  function renderVipPage(direction = 'none') {
    ...
    const doRender = () => {
      ...
      const placeholderUrl = "/media/listing_photos/placeholder.png";
      ...
      pageItems.forEach(item => {
        ...
        if (item.photo) {
          photoUrl = item.photo.startsWith('http') ? item.photo : `${item.photo}`;
        } else {
      ...
      document.querySelectorAll('.favorite-icon').forEach(button => {
          button.addEventListener('click', async (event) => {
            ...
            try {
              ...
              const res = await fetch(`/api/listings/favorites/${id}/toggle/`, {
                method: 'POST',
                headers,
                credentials: 'same-origin',
              });
