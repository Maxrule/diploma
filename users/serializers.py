...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_role(self, value):
        if value == CustomUser.ROLE_ADMIN:
            raise serializers.ValidationError('Admin role can be assigned only in Django admin.')
        return value
    ...
    def create(self, validated_data):
        ...
        role = validated_data.get('role') or CustomUser.ROLE_BUYER
        ...
        if pending_user:
            ...
            user = CustomUser(username=username, email=email, role=role, is_active=False, email_verified=False)
...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = CustomUser
        fields = ['id', 'username', 'email', 'full_name', 'phone', 'role', 'email_verified']
        extra_kwargs = {
            'id': {'read_only': True},
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        data['role'] = self.user.effective_role
        return data
